...
console.log( book.title ) //객체.속성명 = 속성값
...
person2.email = 'coding@gmail.com'
console.log(person2)
...
console.log(`총 자산 가치: ${total}원`)

/* **예제 5: 대괄호 표기법 사용하기**
다음 user 객체가 있습니다. 대괄호 표기법을 사용하여 job-title 속성의 값('개발자')을 콘솔에 출력하시오. */
const user = { name: '김코딩','job-title': '개발자'}
console.log(user['job-title']) //속성명에 특수문자 or 변수로 정의한 경우 객체.속성명 대신 객체['속성명']사용
...
let 객체 = Object.keys(country)
let 값 = Object.values(country)
console.log(객체)
console.log(값)
...
let 배열 =  Object.entries(inventory)
...
let data=response.data //data객체만 저장하기
//html변수는 반복문 바깥에, ``안에 html작성 가능
let html = `<table border> 
            <tr class = "title">
            <td>동별</td><td>세대수</td><td>인구수(계)</td><td>인구수(남)</td><td>인구수(여)</td>
            </tr>`
for(let i=0; i<=data.length-1; i++){
    let 동별객체 = data[i]
    //속성명에 특수문자있으면 객체명['속성명']=속성값
    html+=`<tr>
            <td>${동별객체.동별}</td>
            <td>${동별객체.세대수}</td>
            <td>${동별객체["인구수(계)"]}</td>
